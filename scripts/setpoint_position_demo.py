...
class SetpointPosition:
    ...
    def set(self, x, y, z, ya, delay=0, wait=True):
        self.done = False
        self.x = x
        self.y = y
        self.z = z
...
def setpoint_demo():
    ...
    rospy.loginfo("Tang cao do")
    setpoint.set(0.0, 0.0, 3.0, 0, 0) #(x,y,z,ya,delay)
    setpoint.set(0.0, 0.0, 10.0, 0, 5)

    rospy.loginfo("Ha cao do")
    setpoint.set(0.0, 0.0, 8.0, 0, 5)

    rospy.loginfo("Bay qua phai")
    setpoint.set(10.0, 4.0, 8.0, 0, 5)

    rospy.loginfo("Bay qua trai")
    setpoint.set(0.0, 0.0, 8.0, 0, 5)
    ...
    rospy.loginfo("Bay vong tron")
    setpoint.set(0.0, 0.0, 10.0, 0, 3)   # Climb to the starting height first
    i = 0
    while not rospy.is_shutdown():
        ...
        setpoint.set(x, y, z, ya, delay, wait)

        i = i + 1
        rate.sleep()

        if (i > sides):
            rospy.loginfo("Tro ve vi tri xuat phat")
            setpoint.set(0.0, 0.0, 10.0, 0, 5)
            break

    # Simulate a slow landing.
    setpoint.set(0.0, 0.0,  8.0, 0, 5)
    setpoint.set(0.0, 0.0,  3.0, 0, 5)
    setpoint.set(0.0, 0.0,  2.0, 0, 2)
    setpoint.set(0.0, 0.0,  1.0, 0, 2)
    setpoint.set(0.0, 0.0,  0.0, 0, 2)
    setpoint.set(0.0, 0.0, -0.2, 0, 2)
